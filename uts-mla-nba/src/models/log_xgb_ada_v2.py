...
def objective(params, classifier_type, features_train, target_train, features_val, target_val, preprocessor, trials):
    try:
...
def train_function(train_dataframe, target_column_name, test_size=0.2, random_state=42):
    ...
    best_logreg = fmin(fn=lambda params: objective(params, 'logreg', features_train, target_train, features_val, target_val, preprocessor, trials_logreg),
                   space=space_logreg,
                   algo=tpe.suggest,
                   max_evals=50,
    ...
    best_xgboost = fmin(fn=lambda params: objective(params, 'xgboost', features_train, target_train, features_val, target_val, preprocessor, trials_xgboost),
                   space=space_xgboost,
                   algo=tpe.suggest,
                   max_evals=50,
    ...
    best_adaboost = fmin(fn=lambda params: objective(params, 'adaboost', features_train, target_train, features_val, target_val, preprocessor, trials_adaboost),

...
def train_stacking_ensemble(train_dataframe, target_column_name, test_size=0.2, random_state=42):
    ...
    logreg_probabilities = logreg_pipeline.predict_proba(features_val)[
        :, 1].reshape(-1, 1)
    svm_probabilities = svm_pipeline.predict_proba(features_val)[
        :, 1].reshape(-1, 1)
    ...
    ensemble_probabilities = meta_model.predict_proba(stacked_features_val)[
        :, 1]
